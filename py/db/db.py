...
@author: Jiang
@file name: db
@create date: 2022/9/4 21:29
@description:
"""
from db.mysql_conn_pool import MysqlConn
from utils.log_config import logger


def select(sql: str, return_count=0) -> [int, tuple]:
    """
    zhi
    :param sql:
    :param return_count: 返回行数：0-所有行, 1-一行, 2-两行, ...
    :return:
    """
    logger.info(sql)
    db_conn = MysqlConn()
    try:
        ...
        db_conn.conn.commit()
        rowcount = db_conn.cur.rowcount
        if return_count == 0:
            result = db_conn.cur.fetchall()
        elif return_count == 1:
            result = db_conn.cur.fetchone()
        else:
            result = db_conn.cur.fetchmany(return_count)
        logger.info(f'查询共 {rowcount} 条记录: {str(result)}')
        return rowcount, result
    except Exception as e:
        db_conn.conn.rollback()
        logger.error(f'数据库错误: {str(e)}, 回滚数据库')
        raise e
    finally:
        db_conn.close()


def __execute_sql(sql: str) -> int:
    logger.info(sql)
    db_conn = MysqlConn()
    try:
        db_conn.cur.execute(sql)
...
def update(sql: str) -> int:
    return __execute_sql(sql)


def delete(sql: str) -> int:
    return __execute_sql(sql)


def insert(sql: str) -> int:
    return __execute_sql(sql)
